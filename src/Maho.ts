...
export interface MahoOptions<D extends IData = {}> {
  states: IStatesConfig<D>;
  initial: string;
  data?: D;
  on?: { [key: string]: IEvent<D> | IEvent<D>[] };
...
export class Maho<D extends IData = {}> {
  ...
  constructor(options: MahoOptions<D>, debug = true) {
    const {
      states,
      on,
      actions,
  ...
  private _getSerializedCondition = (condition: string) => {
    ...
    if (conditions === undefined) {
      console.error("This maho doesn't have any serialized conditions.");
      return;
    }
  ...
  private _getSerializedAction = (action: string) => {
    ...
    if (actions === undefined) {
      console.error("This maho doesn't have any serialized actions.");
      return;
    }
  ...
  // Start from the maho's states and drill
...
/*
DATA
A maho maintains an object of arbitrary data. This data is defined in its `data` property. It is may be mutated only through events and actions (see below).

EVENTS (in brief)
An event is dispatched to the maho using the `send` command. Events are dispatched together with a "payload" of arbitrary data. Events may be ignored depending on a) whether the maho is currently capable of "handling" the event, based on its current state (see below), and b) the condition attached to the event. Events have two effects: they may cause the maho to transition into a new state, may mutate the maho's data state, or both. 

STATES
A maho has a finite set of states that the maho may move between  as the result of events and transitions (see below). The states are organized as a "tree", with any state capable of being the parent to other states. At any time, the maho has a "path" of its current states. 

- HANDLING EVENTS
When a maho receives an event (see below), it will only respond if that event is defined in a) the current state's `on` property, b) the `on` property of any state in the "path" of current states, or c) the `on` property of the maho itself. If the event is not "handled" in the current tree, the maho will ignore the event.
...
When the maho receives an event, it will "search" its current path for a state that can respond to the event. It will stop at the first event it finds. In the example aboe, if the maho received a "CANCEL" event while in the "phaseA" state, only the "phaseA" handler would run.

EVENTS
An event is dispatched to the maho using the `send` command. Events are dispatched together with a "payload" of arbitrary data.

Both the maho and its states may have event handlers, define in the `on` object. An `on` object may have any number of event handlers. States may have two additional auto-events (onExit and onEnter) that fire when the mahos transitions into or out of the state.

An event has three parts: a transition, action, and condition. All three parts are optional.

- CONDITIONS
A condition is a function that receives a) the maho's current data state and b) the dispatched payload, and returns a boolean. If a condition is provided and that condition fails to return `true`, then the event will not proceed.

- ACTIONS
An action is a function that receives a) the maho's current data state and b) the dispatched payload, and mutates the data state.

- TRANSITIONS
as either a transition -- the name of a state to transition to, if possible -- and an action, a function that mutates the maho's current data state.
...
SERIALIZATION
Both conditions and actions may be a) defined in the state object or b) defined in a "serialized" form in the maho's "actions" or "conditions" object. In an event, an action or condition function may be replaced by the name of its serialized version.

For example, in the following maho:
...
Serialization has two benefits: a) the "readability" of a maho is increased by using natural language labels for the actions and conditions that govern behavior, and b) serialized actions and conditions may be re-used throughout the maho, reducing repetition and associated errors.
...
In the example above, a battery level of 5 is required to change states to "active", however attempting to "TURN_ON" a maho with less than that amount will still drain the battery.

MULTIPLE EVENTS / CONDITIONS
An event may itself have multiple actions and multiple conditions. In both cases, the multiple actions or conditions are expressed as an array. This is especially useful when using serialized events and conditions (see below), and further helps isolate these parts of the maho into re-usable parts.
...
In the case of multiple actions, each new action will receive the maho's data as mutated by the previous action.

In the case of multiple conditions, all conditions must return true in order for the event to run.

COMPUTED

With the `computed` object, you may specify properties that should be computed, based of the data object and any other available information such as the current date or time. These values are computed each time the maho successfully updates.

Computed values are computed after all transitions and mutations have occurred. They may not mutate the maho's data. (Their mutations will be ignored.)
...
// Expect Maho.computed to equal 7
*/
