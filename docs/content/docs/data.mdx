...
Let's set aside states for a moment and look at the `useMaho` hook as a way of keeping track of a component's stateful data, or the same kind of stuff you'd normally keep track of with a `useState` or `useReducer` hook.
